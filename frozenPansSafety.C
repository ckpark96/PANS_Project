...
tmp<volScalarField>
// frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::frozenInterpPANSkOmegaSST::F1
frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::F1
(
...
) const
{
    ...
    (
        ...
        (
            max
            (
                (scalar(1)/this->betaStar_)*sqrt(kU_LES_)/(omegaU_*y_),
...
frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::F2() const
{
    tmp<volScalarField> arg2 = min
    (
        max
        (
            (scalar(2)/this->betaStar_)*sqrt(kU_LES_)/(omegaU_*y_),
            scalar(500)*(this->mu()/this->rho_)/(sqr(y_)*omegaU_)
        ),
        scalar(100)
    );
...
frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::F3() const
{
...
tmp<volScalarField>
frozenInterpPANSkOmegaSST<BasicEddyViscosityModel>::F23() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix>
frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::kSource() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix>
frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::omegaSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
void frozenInterpPANSkOmegaSST<BasicTurbulenceModel>::correct()
{
    ...
    Info << "TIME: " << this->runTime_.value() << endl;
    ...
    Info << "Calculating tauij_LES" << endl;
    ...
    gradkU_LES_ = fvc::grad(this->kU_LES_);
